...
private:
    std::string                             _name;
    std::vector<shared_ptr<MiddleIRBasicBlock>> _next;
    std::vector<shared_ptr<MiddleIRBasicBlock>> _prev;
...
public:
    ...
    void addNext(shared_ptr<MiddleIRBasicBlock> next_) { _next.emplace_back(std::move(next_)); }
    void addPrev(shared_ptr<MiddleIRBasicBlock> prev_) { _prev.emplace_back(std::move(prev_)); }
    ...
    [[nodiscard]] const std::vector<shared_ptr<MiddleIRBasicBlock>>& getNext() const { return _next; }
    [[nodiscard]] const std::vector<shared_ptr<MiddleIRBasicBlock>>& getPrev() const { return _prev; }
